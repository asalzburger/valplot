...
from utilities.overlay_profiles import (
    parse_band_spread,
    parse_restrict,
    parse_ratio_mode,
    parse_xy_spec,
...
def test_parse_xy_spec():
    assert parse_xy_spec("x:y") == ("x", "y")
    assert parse_xy_spec("  x : y  ") == ("x", "y")
    with pytest.raises(ValueError):
        parse_xy_spec("x")


def test_parse_restrict():
    branch, lo, hi = parse_restrict("y:-4:4")
    assert branch == "y"
    assert lo == -4.0
    assert hi == 4.0

    # reversed should be normalized
    branch, lo, hi = parse_restrict("y:4:-4")
    assert lo == -4.0
    assert hi == 4.0
...
def test_overlay_profiles_runnable_from_anywhere_non_root_file(tmp_path: Path):
    dummy = tmp_path / "dummy.txt"
    dummy.write_text("not a root file")

    script = Path(__file__).resolve().parents[1] / "utilities" / "overlay_profiles.py"

    # Use an arbitrary cwd to ensure sys.path injection works.
    proc = subprocess.run(
        [sys.executable, str(script), "--files", str(dummy), "--input", "tree", "--plot", "x:y"],
        cwd=str(tmp_path),
        capture_output=True,
        text=True,
    )
